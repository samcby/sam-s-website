...
const generatePosition = (skill) => {
  const radius = Math.random() * (skill.maxRadius - skill.minRadius) + skill.minRadius;
  const rotation = Math.random() * (skill.maxAngle - skill.minAngle) + skill.minAngle;
...
const isValidPosition = (position, positions, skills, index, minDistance) => {
  ...
  for (let i = 0; i < index; i++) {
    const x2 = Math.cos((positions[i].rotation * Math.PI) / 180) * positions[i].radius;
    const y2 = Math.sin((positions[i].rotation * Math.PI) / 180) * positions[i].radius;
  ...
  const generateOptimalPositions = useCallback(() => {
    ...
    for (let i = 0; i < DATABASE_SKILLS.length; i++) {
      ...
      while (!isValid && attempts < maxAttempts) {
        ...
        isValid = isValidPosition(position, newPositions, DATABASE_SKILLS, i, MIN_DISTANCE);
        attempts++;
      }
